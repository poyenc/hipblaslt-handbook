...
| `Compatibility with CMake < 3.5 has been removed from CMake` | CMake 4.x dropped support for old `cmake_minimum_required` versions used by some fetched dependencies (e.g. yaml-cpp via RocRoller). Affects any preset that builds the host library with RocRoller enabled. | Add `-DCMAKE_POLICY_VERSION_MINIMUM=3.5` to your cmake command. Alternatively, disable RocRoller with `-DHIPBLASLT_ENABLE_ROCROLLER=OFF` (loses JIT kernel support). `inv build` handles this automatically. |
| `Could not find a package configuration file provided by "LLVM"` | The `rocisa` and `tensilelite` CMake presets enable `HIPBLASLT_ENABLE_YAML=ON`, which requires LLVM development packages for linking against `LLVMObjectYAML`. | Add `-DHIPBLASLT_ENABLE_YAML=OFF` to your cmake command. YAML mode is only needed for developing the YAML config parsing path; the default msgpack path works without it. |
...
| `ld.lld: error: cannot open *.o: Cannot allocate memory` during TensileLite device library build | The linker runs out of memory when linking thousands of kernel `.o` files into a single code object. Even a single architecture (e.g. gfx950 with 617 logic files) can exhaust memory. The `gemm-libs` preset is especially vulnerable because it uses `--no-lazy-library-loading`, linking all code objects into one shared library. | Use `-DTENSILELITE_LOGIC_FILTER` to build a subset (e.g. `-DTENSILELITE_LOGIC_FILTER="gfx950/Equality/*"` for ~30 files). **Note:** `"*"` is the default and has no effect — you must use a specific subdirectory pattern. Other mitigations: reduce parallelism (`cmake --build build --parallel 4`), or add swap (`fallocate -l 16G /swapfile && chmod 600 /swapfile && mkswap /swapfile && swapon /swapfile`). |
